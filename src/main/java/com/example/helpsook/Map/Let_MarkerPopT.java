...
public class Let_MarkerPopT extends Activity {
    // Components
    TextView closeBtn, location_letT, let_take;
    Button check_letT;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_let_marker_pop_t);
        this.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));

        // XML component 와 변수 연결
        location_letT = findViewById(R.id.location_letT);
        let_take = findViewById(R.id.let_take);

        // MainActivity 로부터 전달받은 Qid 를 이용해 퀘스트 내용을 DB 로부터 읽어와 화면에 표시
        Intent get_intent = getIntent();
        String qid = get_intent.getStringExtra("qid");
        FirebaseDatabase database = FirebaseDatabase.getInstance();
        DatabaseReference databaseReference = database.getReference("Content_3");
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    Content_3VO content_3VO = snapshot.getValue(Content_3VO.class);
                    if (content_3VO.getQid().equals(qid)) {
                        location_letT.setText(content_3VO.getCurLocation());
                        let_take.setText(content_3VO.getLecture());
                        break;
                    }
                }
            }
        ...
        // 닫기 버튼 클릭시 수행.
        closeBtn = (TextView) findViewById(R.id.closeBtn2T);
        ...
        check_letT = (Button) findViewById(R.id.check_letT);
        check_letT.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                AlertDialog.Builder ad = new AlertDialog.Builder(Let_MarkerPopT.this);
                            ...
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                                    ...
                                    if (chatRoomVO.getQid().equals(qid) && chatRoomVO.getOfferer().equals(User_Info.uid)) {
                                        Toast.makeText(Let_MarkerPopT.this, "이미 채팅방이 개설되어 있습니다.", Toast.LENGTH_SHORT).show();
                                        exist[0] = true;
                                        break;
                                    }
                                }
