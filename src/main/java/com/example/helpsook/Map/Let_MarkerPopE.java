...
public class Let_MarkerPopE extends Activity {
    // Components
    TextView closeBtn, curLocatioin, let_eat;
    Button check_letE;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_let_marker_pop_e);
        this.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));

        // XML component 와 변수 연결
        curLocatioin = findViewById(R.id.location_letE);
        let_eat = findViewById(R.id.let_eat);

        // MainActivity 로부터 전달받은 Qid 를 이용해 퀘스트 내용을 DB 로부터 읽어와 화면에 표시
        Intent get_intent = getIntent();
        String qid = get_intent.getStringExtra("qid");
        FirebaseDatabase database = FirebaseDatabase.getInstance();
        DatabaseReference databaseReference = database.getReference("Content_2");
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    Content_2VO content_2VO = snapshot.getValue(Content_2VO.class);
                    if (content_2VO.getQid().equals(qid)) {
                        curLocatioin.setText(content_2VO.getCurLocation());
                        let_eat.setText(content_2VO.getMealtype() + "(최대 " + content_2VO.getExpense_max() + "원)");
                        break;
                    }
                }
            }
        ...
        // 닫기 버튼 클릭시 수행.
        closeBtn = (TextView) findViewById(R.id.closeBtn2E);
        ...
        check_letE = (Button) findViewById(R.id.check_letE);
        check_letE.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                AlertDialog.Builder ad = new AlertDialog.Builder(Let_MarkerPopE.this);
                            ...
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                                    ...
                                    if (chatRoomVO.getQid().equals(qid) && chatRoomVO.getOfferer().equals(User_Info.uid)) {
                                        Toast.makeText(Let_MarkerPopE.this, "이미 채팅방이 개설되어 있습니다.", Toast.LENGTH_SHORT).show();
                                        exist[0] = true;
                                        break;
                                    }
                                }
